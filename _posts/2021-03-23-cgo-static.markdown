...
The last '-lcompressor -lz' are exactly flags that I pass to the LDFLAGS in importer.go. '\_x001.o.', '\_x002.o' and \'_x003.o' are object files created from the generated C source code, and \_cgo\_.o' is an executable that doesn't actually do nothing (just returns), but it contains all the symbols necessary for the C part of the program. By default (in gcc), the linker uses dynamic libraries if present, and therefore these libraries can be seen in the .dynamic section's symbol table of the resulting object file. 
